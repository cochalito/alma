...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: `Editar: ${props.departament.code}`, href: '#' },
];

const form = useForm({
    code: props.departament.code,
    location: props.departament.location,
    cost: props.departament.cost,
    status: props.departament.status,
});

function submit() {
    form.put(`/admin/departaments/${props.departament.id}`);
}
</script>

<template>
    <Head :title="`Editar: ${departament.code}`" />
        ...
        <div class="flex flex-col gap-6 p-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold tracking-tight">Editar Departamento</h1>
                        <p class="text-sm text-muted-foreground">{{ departament.code }}</p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="rounded-xl border border-border bg-muted/30 p-4 text-sm space-y-2">
                            ...
                            <div class="flex justify-between text-xs">
                                ...
                                <span class="font-mono font-medium">{{ departament.id }}</span>
                            </div>
                            <div class="flex justify-between text-xs">
                                <span class="text-muted-foreground">Creado</span>
                                <span>{{ new Date(departament.created_at).toLocaleDateString('es-VE') }}</span>
                            </div>
                        </div>
